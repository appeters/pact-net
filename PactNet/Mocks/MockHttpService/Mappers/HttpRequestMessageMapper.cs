        ...
        public HttpRequestMessage Convert(ProviderServiceRequest from)
        {
            ...
            if (from.Headers != null && from.Headers.Any())
            {
                foreach (var requestHeader in from.Headers)
                {
                    ...
                    if (requestHeader.Key.IndexOf("Content-", Constants.StringComparisonCulture) == 0)
                    {
            ...
            if (from.Body != null)
            {
                ...
                if (httpContent != null && contentRelatedHeaders.Any())
                {
                    ...
                    {
                        if (contentHeader.Key.Equals("Content-Type", Constants.StringComparisonCulture) && 
                            httpContent.Headers.Any(x => x.Key.Equals("Content-Type", Constants.StringComparisonCulture)))
                        {
                            continue;
                        }
